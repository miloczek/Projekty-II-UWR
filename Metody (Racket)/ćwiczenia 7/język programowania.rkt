...
(define (list? e)
  (define (list-iter? e)
       (cond [(eq?  e '())]
             [(list-iter? (cdr e))]))
  (and (or (eq? (car e) 'cons)
           (eq? (car e) 'list))
       (list-iter? (cdr e))))

(define (list-cons . es)
   (define (list-cons-iter es)
    (if (eq? es '())
        (null-cons)
        (cons-cons  (car es) (list-cons-iter (cdr es)))))
    (if (eq? es '()) (null-cons)
        (list-cons-iter es)))
...
(define (eval-env e env)
  (cond [(const? e) e]
        ...
        [(null? e)
         (null-cons)]
        [(pair? e)
         (cons (eval-env (pair-fst e) env)
               (eval-env (pair-snd e) env))]
        [(list? e)
         (apply list-cons (cdr e))]
        [(car? e)
