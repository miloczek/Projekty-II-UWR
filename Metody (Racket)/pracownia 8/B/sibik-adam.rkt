...
(define (add-to-env x v env)
  (cons (list 'standard x v) env))


(define (add-expression-to-env name expression env)
  (cons (list 'expression name expression env) env))

(define (find-in-env x env)
  (cond [(null? env) (error "undefined variable" x)]
        [(eq? x (cadar env))
         (cond ((equal? (caar env) 'standard) (caddar env))  ;; if it is not an expression
               ((equal? (caar env) 'expression) (eval-env (caddar env) (cadddr (car env)))))] ;; if it is then evaluate it
...
;; factorial with lazy let
(eval '(let (x (lambda-rec (fact n) (lazy-let [t 1] (lazy-let [f (* n (fact (- n 1)))] (if (= n 0) t f))))) (x 4)))


(eval '(lazy-let (x 10) (let (x 5) x)))
