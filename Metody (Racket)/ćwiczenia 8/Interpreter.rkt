...
(define (my-list-cons . e)
  (cond [(null? e) (my-null-cons)]
        [else (cons-cons (car e) (apply my-list-cons (cdr e)))]))
...
(define (eval-env e env)
  (define (eval-with-current-env e)
    (eval-env e env))
  (cond [(const? e)
         e]
        [(op? e)
        ...
        [(my-null? e)
         null]
        [(my-list? e)
          (eval-env (my-list-cons (cdr e)) env)]
        [(cons? e)
        ...
        [(lambda-rec? e)
         (closure-rec-cons (lambda-rec-name e)
                           ...
                           env)]
        [(expr-append? e)
         (eval-env (list (expr-append) (second e) (third e)) env)]
        [(app? e)
         (apply-closure
